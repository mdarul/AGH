...
--d
CREATE OR REPLACE VIEW WYCIECZKI_MIEJSCA AS SELECT
 w.ID_WYCIECZKI,
 w.NAZWA,
 w.KRAJ,
 w.LICZBA_MIEJSC,
 w.LICZBA_MIEJSC - (SELECT COUNT(*) FROM REZERWACJE r WHERE w.ID_WYCIECZKI = r.ID_WYCIECZKI AND r.STATUS != 'A') as wolne_miejsca
  FROM WYCIECZKI w;
...
CREATE VIEW DOSTEPNE_WYCIECZKI AS SELECT
 ...
 w.LICZBA_MIEJSC - (SELECT COUNT(*) FROM REZERWACJE r WHERE w.ID_WYCIECZKI = r.ID_WYCIECZKI AND r.STATUS != 'A') as wolne_miejsca
  FROM WYCIECZKI w
  WHERE  w.LICZBA_MIEJSC - (SELECT COUNT(*) FROM REZERWACJE r WHERE w.ID_WYCIECZKI = r.ID_WYCIECZKI AND r.STATUS != 'A') > 0
